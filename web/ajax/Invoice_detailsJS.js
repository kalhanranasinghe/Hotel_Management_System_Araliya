...
function selectedRowToInput() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function () {
            ...
            document.getElementById("stock_id").value = this.cells[3].textContent;
            document.getElementById("qty").value = this.cells[4].textContent;
            document.getElementById("total").value = this.cells[5].textContent;
...
function save() {
    ...
    var action = "insert";
    if (invoice_details_id === "" || invoice_id === "" || stock_id === "" || qty === "" || total === "" || date_time === "") {
        alert("Please Enter All Details")
    } else {
        $.ajax({
            url: 'Invoice_detailsServlet',
            method: 'POST',
            data: {action: action, invoice_details_id: invoice_details_id, invoice_id: invoice_id, stock_id: stock_id, qty: qty, total: total, date_time: date_time},
...
function update() {
    ...
    } else if (invoice_details_id === "" || invoice_id === "" || stock_id === "" || qty === "" || total === "" || date_time === "") {
        alert("Please Enter All Details")
    } else {
        $.ajax({
            url: 'Invoice_detailsServlet',

...
function selectedRowToInput() {
    ...
    for (var i = 1; i < table.rows.length; i++) {
        table.rows[i].onclick = function () {
            ...
            document.getElementById("invoice_id").value = this.cells[1].textContent;
            document.getElementById("customer_id").value = this.cells[2].textContent;
            document.getElementById("bonus").value = this.cells[3].textContent;
...
function save() {
    ...
    var action = "insert";
    if (invoice_id === "" || customer_id === "" || bonus === "" || price === "" || date_time === "") {
        alert("Please Enter All Details")
    } else {
        $.ajax({
            url: 'InvoiceServlet',
            method: 'POST',
            data: {action: action, invoice_id: invoice_id, customer_id: customer_id, bonus: bonus, price: price, date_time: date_time},
...
function update() {
    ...
    } else if (invoice_id === "" || customer_id === "" || bonus === "" || price === "" || date_time === "") {
        alert("Please Enter All Details")
    } else {
        $.ajax({
            url: 'InvoiceServlet',
    ...
    if (invoice_id === 0) {
        ...
        if (r == true) {
            $.ajax({
                url: 'InvoiceServlet',
                method: 'POST',
                data: {action: action, invoice_id: invoice_id},
                success: function (resultText) {
                    $("#table").find("tr:gt(0)").remove();
                    load();
                    alert("Deleted")
